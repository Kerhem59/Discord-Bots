...
function createEmbed(options = {}) {
  ...
  // Helper: resolve guild name from passed context
  const guildName = options.context?.guild?.name || options.context?.name || null;
  ...
  // Footer resolution (supports placeholders like "{guild}")
  let footerText = options.footer ?? defaultSettings.footer;
  if (footerText && typeof footerText === 'string' && footerText.includes('{guild}') && guildName) {
    footerText = footerText.replace('{guild}', guildName);
  }
  if ((!footerText || String(footerText).trim() === '') && guildName) {
  ...
  // Yazar (supports placeholder)
  let authorName = options.authorName ?? defaultSettings.authorName;
  if (authorName && typeof authorName === 'string' && authorName.includes('{guild}') && guildName) {
    authorName = authorName.replace('{guild}', guildName);
  }
  if (authorName) {
    embed.setAuthor({
      name: authorName,
